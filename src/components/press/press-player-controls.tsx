...
export function ProgressBar({
  ...
  const thumbLeft = `calc(${value * 100}% - ${touch ? 8 : 5}px)`;
  const heightClass = touch ? "h-10" : "h-3";
  const thumbClass = touch ? "h-4 w-4" : "h-2.5 w-2.5";
...
export function GlassTransportButton({
  ...
  return (
    ...
    >
      ...
      />
      <span
        className={`pointer-events-none absolute inset-[1px] border border-white/10 ${large ? "rounded-[7px]" : "rounded-[5px]"}`}
        aria-hidden
      />
      <span
        className={`relative z-10 flex h-full items-center justify-center gap-2 uppercase tracking-[0.22em] text-white/90 ${
          large ? "px-4 text-sm" : "px-2 text-[10px]"
        }`}
      >
        <span className={large ? "text-xs leading-none" : "text-[9px] leading-none"}>
          {isPlaying ? "⏹" : "▶"}
        </span>
        <span>{label}</span>
      </span>
...
export function HorizontalVolume({
  ...
  return (
    <label className="flex items-center gap-3 text-[10px] uppercase tracking-widest text-white/50">
      ...
      <input
        ...
        value={volume}
        onChange={(e) => onChange(Number(e.target.value))}
        className="h-10 min-w-0 flex-1 cursor-pointer touch-manipulation"
