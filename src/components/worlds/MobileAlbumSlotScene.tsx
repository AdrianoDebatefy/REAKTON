...
type ExitOverlaySnapshot = {
  covers: ExitCoverRect[];
};
...
const EXIT_HANDOFF_BUFFER_MS = 600;
...
export function MobileAlbumSlotScene({
  ...
  const [exitOverlay, setExitOverlay] = useState<ExitOverlaySnapshot | null>(null);
  const [exitFramerFallback, setExitFramerFallback] = useState(false);
  ...
  const exitTotalMs = useMemo(
    () => mobileCoverExitMs(coverExitDelays, MOBILE_COVER_FADE_DURATION_S, EXIT_HANDOFF_BUFFER_MS),
    [coverExitDelays]
  );
  ...
  const useExitOverlay = exiting && exitOverlay !== null;
  const fadeWithFramer = exiting && exitFramerFallback && !useExitOverlay;
  ...
  useLayoutEffect(() => {
    if (!exiting) {
      exitLayoutRef.current = null;
      setExitOverlay(null);
      setExitFramerFallback(false);
      return;
    }
    if (exitOverlay || exitFramerFallback) return;

    const scene = sceneRef.current;
    if (!scene) return;

    const covers: ExitCoverRect[] = [];
    items.forEach((song, index) => {
      const hidden = isPoleMode && activeId !== song.id;
      if (hidden) return;
      const el = scene.querySelector<HTMLElement>(`[data-cover-id="${song.id}"]`);
      if (!el) return;
    ...
    if (covers.length > 0) {
      setExitOverlay({ covers });
    } else {
      setExitFramerFallback(true);
    }
  }, [activeId, exitFramerFallback, exitOverlay, exiting, isPoleMode, items]);
  ...
  const exitOverlayPortal =
    useExitOverlay && portalReady
      ? createPortal(
          <div className="pointer-events-none fixed inset-0 z-[35]" aria-hidden>
            {exitOverlay.covers.map((cover) => {
              const song = items.find((item) => item.id === cover.songId);
              if (!song) return null;
              const exitDelay = coverExitDelays[cover.index] ?? 0;
              return (
                <div
                  key={`exit-${cover.songId}`}
                  className={`fixed overflow-hidden rounded-sm border ${borderClass} ${
                    cover.isActive
                  ...
                  style={{
                    ...
                    backgroundColor:
                      song.coverImage && !song.coverImage.includes("placeholder")
                        ...
                        : "#C1E5F9",
                    opacity: 1,
                    animation: `mobile-cover-exit-fade ${MOBILE_COVER_FADE_DURATION_S}s ease ${exitDelay}s forwards`,
                    willChange: "opacity",
                  }}
                  onAnimationEnd={(event) => {
                    if (!event.animationName.includes("mobile-cover-exit-fade")) return;
                    handleCoverFadeComplete(cover.songId);
                  }}
                >
                  <div className="relative h-full w-full bg-[#080c12]">
                    <Image
                      ...
                      sizes={cover.isActive ? "390px" : "60px"}
                      draggable={false}
                    />
                    {!cover.isActive && (
  ...
  return (
    <>
      <div
        ref={sceneRef}
        className="album-slot-scene relative z-20 mx-auto h-[calc(100svh-11.5rem-env(safe-area-inset-top))] min-h-[320px] w-full max-w-lg overflow-hidden bg-transparent"
        style={{
          isolation: "isolate",
          transform: `translateY(-${MOBILE_COVER_BLOCK_SHIFT_PX}px)`,
          visibility: useExitOverlay ? "hidden" : "visible",
        }}
      >
        {items.map((song, i) => {
          const pad = padPositions[i] ?? MOBILE_COVER_ACTIVE_CENTER;
          ...
          return (
            <motion.button
              ...
              animate={{
                ...
                x: "-50%",
                y: "-50%",
                opacity: hidden ? 0 : fadeWithFramer ? 0 : 1,
                scale: fadeWithFramer ? restingScale : hidden ? 0.85 : 1,
              }}
              transition={
                fadeWithFramer
                  ? {
