"use client";

import { useCallback, useEffect, useRef, useState } from "react";
import Image from "next/image";
import { motion, AnimatePresence } from "framer-motion";
import { useTranslations } from "next-intl";
import type { Locale, Song } from "@/types/content";
import { getLocalized } from "@/lib/locale";

function getYouTubeId(url: string): string | null {
  const trimmed = url.trim();
  const patterns = [
    /(?:youtu\.be\/|youtube\.com\/embed\/|youtube\.com\/shorts\/)([^?&/]+)/,
    /[?&]v=([^?&/]+)/,
  ];
  for (const pattern of patterns) {
    const match = trimmed.match(pattern);
    if (match?.[1]) return match[1];
  }
  return null;
}

const INFO_TEXT_SIZE_KEY = "reakton-info-text-size";
const INFO_TEXT_SIZE_MIN = 12;
const INFO_TEXT_SIZE_MAX = 22;
const INFO_TEXT_SIZE_STEP = 2;
const INFO_TEXT_SIZE_DEFAULT = 14;

function readStoredInfoTextSize(): number {
  try {
    const saved = localStorage.getItem(INFO_TEXT_SIZE_KEY);
    if (saved) {
      const size = Number(saved);
      if (size >= INFO_TEXT_SIZE_MIN && size <= INFO_TEXT_SIZE_MAX) return size;
    }
  } catch {
    /* ignore */
  }
  return INFO_TEXT_SIZE_DEFAULT;
}

export function MobileAlbumSlotScene({
  songs,
  maxSlots = 12,
  borderClass = "border-white/30",
  exiting = false,
  onExitComplete,
  locale = "de",
}: {
  songs: Song[];
  maxSlots?: number;
  borderClass?: string;
  exiting?: boolean;
  onExitComplete?: () => void;
  locale?: Locale;
}) {
  const t = useTranslations("world");
  const items = songs.slice(0, maxSlots);
  const [activeId, setActiveId] = useState<string | null>(null);
  const [infoPanelOpen, setInfoPanelOpen] = useState(false);
  const [infoTextSize, setInfoTextSize] = useState(INFO_TEXT_SIZE_DEFAULT);
  const [youtubeUrl, setYoutubeUrl] = useState<string | null>(null);
  const [volume, setVolume] = useState(0.7);
  const audioRef = useRef<HTMLAudioElement | null>(null);
  const slotVideoRef = useRef<HTMLVideoElement | null>(null);
  const exitStartedRef = useRef(false);

  const activeSong = items.find((s) => s.id === activeId) ?? null;
  const activeInfoText = activeSong ? getLocalized(activeSong.infoText, locale) : "";

  useEffect(() => {
    setInfoTextSize(readStoredInfoTextSize());
  }, []);

  const stopAudio = useCallback(() => {
    if (audioRef.current) {
      audioRef.current.pause();
      audioRef.current = null;
    }
  }, []);

  const startAudio = useCallback(
    (song: Song) => {
      if (!song.audioSnippet) return;
      const audio = new Audio(song.audioSnippet);
      audio.loop = true;
      audio.volume = volume;
      audioRef.current = audio;
      audio.play().catch(() => undefined);
    },
    [volume]
  );

  const handleSelect = useCallback(
    (song: Song) => {
      if (exiting) return;
      if (activeId === song.id) return;
      stopAudio();
      setActiveId(song.id);
      setInfoPanelOpen(false);
      startAudio(song);
    },
    [activeId, exiting, startAudio, stopAudio]
  );

  const handleBackToGrid = useCallback(() => {
    stopAudio();
    slotVideoRef.current?.pause();
    setActiveId(null);
    setInfoPanelOpen(false);
  }, [stopAudio]);

  const adjustInfoTextSize = useCallback((delta: number) => {
    setInfoTextSize((prev) => {
      const next = Math.min(INFO_TEXT_SIZE_MAX, Math.max(INFO_TEXT_SIZE_MIN, prev + delta));
      try {
        localStorage.setItem(INFO_TEXT_SIZE_KEY, String(next));
      } catch {
        /* ignore */
      }
      return next;
    });
  }, []);

  useEffect(() => {
    if (!exiting) {
      exitStartedRef.current = false;
      return;
    }
    if (exitStartedRef.current) return;
    exitStartedRef.current = true;
    stopAudio();
    const timer = window.setTimeout(() => onExitComplete?.(), 420);
    return () => window.clearTimeout(timer);
  }, [exiting, onExitComplete, stopAudio]);

  useEffect(() => () => stopAudio(), [stopAudio]);

  return (
    <motion.div
      className="relative z-10 px-3 pb-8 pt-2"
      animate={{ opacity: exiting ? 0 : 1 }}
      transition={{ duration: 0.4 }}
    >
      <AnimatePresence mode="wait">
        {activeId && activeSong ? (
          <motion.div
            key="active"
            initial={{ opacity: 0, y: 12 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 8 }}
            transition={{ duration: 0.35 }}
            className="mx-auto max-w-md"
          >
            <button
              type="button"
              onClick={handleBackToGrid}
              className="mb-3 text-sm uppercase tracking-widest text-white/55"
            >
              ← {t("backToGrid")}
            </button>

            <div
              className={`relative aspect-square w-full overflow-hidden rounded-sm border ${borderClass}`}
            >
              {activeSong.videoSnippet ? (
                <video
                  ref={slotVideoRef}
                  src={activeSong.videoSnippet}
                  className="h-full w-full object-cover"
                  autoPlay
                  loop
                  playsInline
                  muted={!!activeSong.audioSnippet}
                />
              ) : (
                <Image
                  src={activeSong.coverImage || "/covers/placeholder.svg"}
                  alt={activeSong.title}
                  fill
                  className="object-cover"
                  sizes="100vw"
                  priority
                />
              )}

              {!infoPanelOpen && (
                <button
                  type="button"
                  onClick={() => setInfoPanelOpen(true)}
                  className="cover-info-btn absolute right-2 top-2 z-20"
                  aria-label={t("songInfo")}
                >
                  <span className="cover-info-btn__glyph">i</span>
                </button>
              )}

              <AnimatePresence>
                {infoPanelOpen && (
                  <motion.div
                    key="info"
                    initial={{ x: "100%" }}
                    animate={{ x: 0 }}
                    exit={{ x: "100%" }}
                    transition={{ duration: 0.32, ease: [0.4, 0, 0.2, 1] }}
                    className="cover-info-panel absolute inset-0 z-30 flex flex-col"
                  >
                    <div className="cover-info-panel__toolbar flex shrink-0 items-center justify-between gap-2 px-3 py-2">
                      <div className="flex items-center gap-1.5">
                        <button
                          type="button"
                          onClick={() => adjustInfoTextSize(-INFO_TEXT_SIZE_STEP)}
                          disabled={infoTextSize <= INFO_TEXT_SIZE_MIN}
                          className="cover-info-size-btn"
                          aria-label={t("textSizeDecrease")}
                        >
                          −
                        </button>
                        <button
                          type="button"
                          onClick={() => adjustInfoTextSize(INFO_TEXT_SIZE_STEP)}
                          disabled={infoTextSize >= INFO_TEXT_SIZE_MAX}
                          className="cover-info-size-btn"
                          aria-label={t("textSizeIncrease")}
                        >
                          +
                        </button>
                      </div>
                      <button
                        type="button"
                        onClick={() => setInfoPanelOpen(false)}
                        className="cover-info-panel__close px-2 py-1 text-[11px] uppercase tracking-[0.25em] text-black/55"
                      >
                        {t("closeInfo")}
                      </button>
                    </div>
                    <div
                      className="cover-info-panel__body flex-1 overflow-y-auto px-5 pb-5 pt-1 leading-relaxed text-black/88"
                      style={{ fontSize: `${infoTextSize}px` }}
                    >
                      {activeInfoText ? (
                        <p className="whitespace-pre-wrap">{activeInfoText}</p>
                      ) : (
                        <p className="text-black/45">{t("noSongInfo")}</p>
                      )}
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
            </div>

            <div className="mt-4 space-y-3 rounded border border-white/15 bg-black/35 p-4 backdrop-blur-sm">
              {activeSong.audioSnippet && (
                <label className="flex items-center gap-3 text-xs text-white/70">
                  <span className="uppercase tracking-widest">{t("nowPlaying")}</span>
                  <input
                    type="range"
                    min={0}
                    max={1}
                    step={0.01}
                    value={volume}
                    onChange={(e) => {
                      const next = parseFloat(e.target.value);
                      setVolume(next);
                      if (audioRef.current) audioRef.current.volume = next;
                    }}
                    className="min-w-0 flex-1"
                  />
                </label>
              )}
              {activeSong.videoUrl && (
                <button
                  type="button"
                  onClick={() => {
                    stopAudio();
                    setYoutubeUrl(activeSong.videoUrl!.trim());
                  }}
                  className="block w-full text-center text-sm uppercase tracking-[0.3em] text-white/85 underline"
                >
                  {t("watchVideo")}
                </button>
              )}
              <p className="text-center text-lg lowercase tracking-[0.15em] text-white">
                {activeSong.title}
              </p>
            </div>
          </motion.div>
        ) : (
          <motion.div
            key="grid"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="grid grid-cols-2 gap-3 sm:grid-cols-3"
          >
            {items.map((song) => (
              <button
                key={song.id}
                type="button"
                disabled={exiting}
                onClick={() => handleSelect(song)}
                className={`relative aspect-square overflow-hidden rounded-sm border text-left ${borderClass} shadow-md shadow-black/40`}
              >
                <Image
                  src={song.coverImage || "/covers/placeholder.svg"}
                  alt={song.title}
                  fill
                  className="object-cover"
                  sizes="50vw"
                />
                <span className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/85 to-transparent px-2 py-2 text-[11px] uppercase tracking-wider text-white/90">
                  {song.title}
                </span>
              </button>
            ))}
          </motion.div>
        )}
      </AnimatePresence>

      <AnimatePresence>
        {youtubeUrl && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[60] flex items-center justify-center bg-black/90 p-4 pt-24"
            role="dialog"
            aria-modal
          >
            <button
              type="button"
              onClick={() => setYoutubeUrl(null)}
              className="absolute right-4 top-28 text-sm uppercase tracking-widest text-white/70"
            >
              {t("closeVideo")}
            </button>
            <div className="aspect-video w-full max-w-lg">
              {getYouTubeId(youtubeUrl) ? (
                <iframe
                  title="Video"
                  src={`https://www.youtube-nocookie.com/embed/${getYouTubeId(youtubeUrl)}?autoplay=1&rel=0`}
                  className="h-full w-full"
                  allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                  allowFullScreen
                />
              ) : (
                <p className="text-center text-white/60">Ungültiger YouTube-Link</p>
              )}
            </div>
          </motion.div>
        )}
      </AnimatePresence>
    </motion.div>
  );
}
